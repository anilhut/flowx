...
const TypeTwo: React.FC<CustomNodeProps> = (props) => {
  ...
  const { title, value1_header, value1_data, value2_header, value2_data, bgColorCondition, hasChildren, isCollapsed, onToggleCollapse } = props.data;

  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <div className={`${styles.nodeCustomContainer} ${styles.nodeTypeTwo} ${getBgClass(styles, bgColorCondition)}`}>
        ...
        <div className={styles.nodeCustomValues}>
          {title && (
            <div className={styles.nodeCustomTitle} title={title}>
              <b>{title}</b>
            </div>
          )}
